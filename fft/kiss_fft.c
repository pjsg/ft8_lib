...
#else
static void kf_bfly4(kiss_fft_cpx *restrict Fout, const size_t fstride,
                     const kiss_fft_cfg st, const size_t m) {
  const kiss_fft_cpx *restrict tw = st->twiddles;
  const size_t m2 = 2 * m;
  const size_t m3 = 3 * m;

  // The 'restrict' keyword is vital here; it tells the compiler
  // that Fout and twiddles do not overlap in memory.
  ...
  for (size_t k = 0; k < m; ++k) {
    ...
    // Use indexing instead of pointer increments (++Fout)
    // This makes the 'reduction' error disappear because Fout[k]
    // is clearly independent for each k.
    kiss_fft_cpx f0 = Fout[k];
    kiss_fft_cpx f1 = Fout[k + m];
    kiss_fft_cpx f2 = Fout[k + m2];
    kiss_fft_cpx f3 = Fout[k + m3];
    ...
    // Fetch twiddles using the stride index
    C_MUL(scratch[0], f1, tw[k * fstride]);
    C_MUL(scratch[1], f2, tw[k * fstride * 2]);
    C_MUL(scratch[2], f3, tw[k * fstride * 3]);

    C_SUB(scratch[5], f0, scratch[1]);
    C_ADDTO(f0, scratch[1]);
    C_ADD(scratch[3], scratch[0], scratch[2]);
    C_SUB(scratch[4], scratch[0], scratch[2]);

    Fout[k + m2].r = f0.r - scratch[3].r;
    Fout[k + m2].i = f0.i - scratch[3].i;
    C_ADDTO(f0, scratch[3]);
    Fout[k] = f0;
